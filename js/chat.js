...
async function acceptFriendRequest(friendUid, friendName, friendPhoto, friendTag) {
  ...
  try {
    ...
    batch.set(_fdb().collection('users').doc(_uid()).collection('friends').doc(friendUid), {
      uid: friendUid,
    ...
    batch.set(_fdb().collection('users').doc(friendUid).collection('friends').doc(_uid()), {
      uid: _uid(),
      displayName: window.TF_USER?.displayName || me.displayName || 'Friend',
      photoURL: window.TF_USER?.photoURL || me.photoURL || '',
      friendTag: me.friendTag || '',
    ...
    // Clean up only the two real request records:
    // 1) receiver's inbox request, 2) sender's outgoing request.
    // Do not delete extra mirrored paths because strict Firebase rules may block them.
    batch.delete(_fdb().collection('users').doc(_uid()).collection('friend_requests').doc(friendUid));
    batch.delete(_fdb().collection('users').doc(friendUid).collection('sent_friend_requests').doc(_uid()));
